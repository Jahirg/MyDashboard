...
# CUSTOM LEFT MENU
class TopBarButtonExtra(QWidget):
	# SIGNALS
	clicked = pyqtSignal()
	released = pyqtSignal()
	...
	def __init__(self, id, custom_btn, btn_image, btn_description, is_active, parent):
		...
		# BG COLORS
		self.color_default = QColor(self.settings["top_menu_extra"]["color_idle"])
		self.color_hover = QColor(self.settings["top_menu_extra"]["color_hover"])
		self.color_pressed = QColor(self.settings["top_menu_extra"]["color_pressed"])
		self.color_actived = QColor(self.settings["top_menu_extra"]["color_actived"])
		self._set_color = self.color_default

		# ICON
		self.icon_color_default = QColor(self.settings["top_menu_extra"]["icon_color_idle"])
		self.icon_color_hover = QColor(self.settings["top_menu_extra"]["icon_color_hover"])
		self.icon_color_pressed = QColor(self.settings["top_menu_extra"]["icon_color_pressed"])
		self.icon_color_actived = QColor(self.settings["top_menu_extra"]["icon_color_actived"])
		self._set_icon_color = self.icon_color_default
		self._set_icon_path = icon_path        

		# TOOLTIP
		# TOOLTIP / LABEL StyleSheet
		tooltip_bg_color = self.settings["top_menu_extra"]["tooltip_bg"]
		tooltip_boder_color = self.settings["top_menu_extra"]["tooltip_border"]
		style_tooltip_basic = """ 
		QLabel {
			color: rgb(0, 250, 250);
			padding-left: 10px;
	...
	def icon_paint(self, qp, image, rect, color):
		icon = QPixmap(image)
		icon = icon.scaled(28, 28, Qt.KeepAspectRatio, Qt.SmoothTransformation) # resize ICONS
	...
	def move_tooltip(self):
		...
		pos_x = pos.x() 
		pos_y = pos.y() + 40 + (self.width - self.tooltip.height()) // 2
